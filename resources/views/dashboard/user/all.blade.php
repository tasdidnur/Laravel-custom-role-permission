          ...
          <div class="card-header">
            ...
            <div class="card-tools">
              <div class="input-group input-group-sm" style="width: 150px;">
                @isset(Auth::user()->roleInfo->permissionInfo['permission']['user']['add'])
            ...
            <table id="myTable" class="table table-bordered table-hover text-nowrap">
              ...
              @foreach($allUser as $data)
                <tr>
                  ...
                  <td>{{ $data->email }}</td>
                  <td>{{ $data->phone }}</td>
                  <td>{{ optional($data->roleInfo)->name }}</td>
                  <td>@if($data->image!='')
                  ...
                  <td>
                    <div class="btn-group">
                      ...
                      <div class="dropdown-menu" role="menu">
                        @isset(Auth::user()->roleInfo->permissionInfo['permission']['user']['view'])
                        <a class="dropdown-item" href="{{ url('dashboard/user/view/'.$data->id) }}">View</a>
                        @endisset
                        @isset(Auth::user()->roleInfo->permissionInfo['permission']['user']['edit'])
                        <a class="dropdown-item" href="{{ url('dashboard/user/edit/'.$data->id) }}">Edit</a>
                        @endisset
                        @isset(Auth::user()->roleInfo->permissionInfo['permission']['user']['delete'])
